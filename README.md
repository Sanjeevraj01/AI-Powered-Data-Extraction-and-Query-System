...
The application uses NVIDIA NIM for answer generation.
...
The application uses Pinecone as the vector database.
...
```text
backend/services/pinecone_db.py
```

## Embedding Configuration

Document chunks and user queries are embedded using:
...
Text is split into smaller chunks before embedding generation.
...
Relevant chunks are retrieved from Pinecone using similarity search.
...
The retrieved context is sent to NVIDIA NIM to generate the final response.
